...
export default function SystemMetricChart({ 
  title, 
  data, 
  unit = '%',
  maxValue = 100,
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-popover border border-border rounded-lg p-2 shadow-lg">
          <p className="text-xs font-semibold">{payload[0].value.toFixed(1)}{unit}</p>
        </div>
      );
    }
    return null;
  ...
  return (
    <Card>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="flex items-baseline gap-3 mt-1">
              ...
              <span className="text-xs text-muted-foreground">max: {maxVal.toFixed(1)}{unit}</span>
            </div>
          </div>
      ...
      <CardContent>
        ...
        <div className={zoomMode ? 'cursor-crosshair' : ''} style={{ height: `${height}px` }}>
